...
import { Loader2, Zap, ChevronLeft, ChevronRight, Play, Star } from 'lucide-react';
...
export function NovidadesSection() {
  ...
  const { data: items = [], isLoading } = useQuery({
    queryKey: ['home-novidades'],
    queryFn: async (): Promise<Item[]> => {
      const [m, s] = await Promise.all([
        supabase.from('movies').select('id,title,year,rating,image_url,created_at').order('created_at', { ascending: false }).limit(15),
        supabase.from('series').select('id,title,year,rating,image_url,created_at').order('created_at', { ascending: false }).limit(15),
      ]);
      const all: Item[] = [
        ...(m.data || []).map((i): Item => ({ id: i.id, title: i.title, year: i.year || '', rating: i.rating ?? 0, imageUrl: i.image_url || '/placeholder.svg', type: 'movie', createdAt: i.created_at })),
        ...(s.data || []).map((i): Item => ({ id: i.id, title: i.title, year: i.year || '', rating: i.rating ?? 0, imageUrl: i.image_url || '/placeholder.svg', type: 'series', createdAt: i.created_at })),
      ];
      return all.sort((a, b) => (a.createdAt < b.createdAt ? 1 : -1)).slice(0, 20);
    },
  ...
  return (
    <section id="novidades" className="py-12 md:py-16 relative">
          ...
          <div className="relative group/carousel">
            ...
            >
              {items.map((item, i) => {
                ...
                return (
                  <Link key={item.id} to={href} className="snap-start shrink-0 w-[160px] sm:w-[180px] md:w-[200px] group/card block">
                    <div className="relative aspect-[2/3] rounded-xl overflow-hidden bg-secondary ring-1 ring-border/20 group-hover/card:ring-primary/50 transition-all duration-500 shadow-md">
                      <img
                        src={item.imageUrl}
                        alt={item.title}
                      ...
                      <span className="absolute top-2 left-2 bg-primary/90 backdrop-blur-sm text-primary-foreground text-[10px] font-bold uppercase px-2 py-0.5 rounded-md tracking-wider">
                        Novo
                      </span>
                      {item.rating > 0 && (
