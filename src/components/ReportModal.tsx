...
import { X, AlertTriangle, Loader2, LogIn, LifeBuoy } from 'lucide-react';
...
export function ReportModal({ contentId, contentType, contentTitle, open, onClose }: ReportModalProps) {
  ...
  const handleSend = useCallback(async () => {
    ...
    toast.success('Ticket aberto! Acompanhe pela página de Suporte.');
    setReason('');
    setDetails('');
    setSending(false);
    onClose();
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="flex items-start gap-2 px-3 py-2 bg-primary/10 border border-primary/30 rounded-lg">
                ...
                <p className="text-[11px] text-foreground/80">
                  Você poderá acompanhar e responder este ticket na página de{' '}
                  <Link to="/suporte" className="text-primary font-semibold hover:underline">Suporte</Link>.
                </p>
              </div>
            </div>
