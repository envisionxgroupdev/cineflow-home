...
export function TelegramManagement() {
  ...
  const addChannel = () => setChannels(prev => [...prev, { id: crypto.randomUUID(), name: "", chatId: "", type: "all" }]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const [, key] of Object.entries(keysToSave)) {
        await supabase.from("site_settings").upsert({ key, value: config[key] || "" }, { onConflict: "key" });
      }
      await supabase.from("site_settings").upsert({ key: TELEGRAM_KEYS.channels, value: JSON.stringify(channels) }, { onConflict: "key" });
